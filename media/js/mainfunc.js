...
function showqq() {
  ...
  if (qq != '') window.location.href = 'tencent://message/?uin=' + qq + '&Site=&Menu=yes';
  else alert('博主暂未设置QQ联系方式');
}

// --------------点击按钮显示Aplayer👇--------------
function showAaplayer() {
  var aplayer = $('.aplayer');
  if (aplayer.css('display') == 'block') aplayer.fadeOut(200);
  else aplayer.fadeIn(200);
}
// --------------点击按钮显示Aplayer👆--------------
...
function getHotnum() {
  ...
  if (pl.search('post') == -1) return false;
  if (pl[pl.length - 1] != '/') pl += '/';
...
async function codebtncolorinit(e) {
  ...
  e.trigger.style.background = '#636363';
}
...
function codeinit() {
  ...
  cop.on('success', function (e) {
    ...
    e.trigger.style.background = '#108715';
    codebtncolorinit(e);
    e.clearSelection();
  });
